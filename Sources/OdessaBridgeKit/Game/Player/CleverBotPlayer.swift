//
//  CleverBotPlayer.swift
//
//
//  Created by Anatoliy Khramchenko on 03.01.2024.
//
...
class CleverBotPlayer: Player {
    ...
    func chooseSuit() async -> CardSuit {
        let suits = [CardSuit.cross, .peak, .chirva, .tambourine]
        let counts = suits.map({ suit in cardsInHand.filter({ $0.suit.contains(suit) }).count })
        let max = counts.max()!
        let index = counts.firstIndex(of: max)!
        return suits[index]
    }
    ...
    func chooseCards() async -> [Card]? {
        ...
        for suit in suits {
            if let card = cardsInHand.first(where: { $0.suit.contains(suit) && $0.value == .six }) {
            ...
            if let card = cardsInHand.first(where: { $0.suit.contains(suit) && $0.value == .seven }) {
                let puttedCards = cardsInHand.filter({ card.value == $0.value })
                for puttedCard in puttedCards {
                    cardsInHand.removeAll(where: { $0 == puttedCard })
                }
            ...
            if let card = cardsInHand.first(where: { $0.suit.contains(suit) && $0.value == .ace }) {
                let puttedCards = cardsInHand.filter({ card.value == $0.value })
                for puttedCard in puttedCards {
                    cardsInHand.removeAll(where: { $0 == puttedCard })
                }
            ...
            if let card = cardsInHand.first(where: { $0.suit.contains(suit) && $0.value == .king }) {
                let puttedCards = cardsInHand.filter({ card.value == $0.value })
                for puttedCard in puttedCards {
                    cardsInHand.removeAll(where: { $0 == puttedCard })
                }
            ...
            if let card = cardsInHand.first(where: { $0.suit.contains(suit) && $0.value == .eight }) {
                let puttedCards = cardsInHand.filter({ card.value == $0.value })
                for puttedCard in puttedCards {
                    cardsInHand.removeAll(where: { $0 == puttedCard })
                }
